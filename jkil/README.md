...
On Windows, the AutoHotkey v2 setup uses a remapped layer key: physical `Right Alt` is remapped to `AppsKey` first, then used as the internal `jkil` layer key. This avoids `AltGr`-related issues on some keyboard layouts while keeping the same physical hotkey.
...
* additional keys difficult to reach or absent on some keyboards (`PgUp`, `PgDown`, `Home`, `End`, `Delete`, `Backspace`, `Enter`)
